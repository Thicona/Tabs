...
export default function Home({ route, navigation }: Props) {
  ...
  const renderPetItem = ({ item }: { item: typeof PETS_DATA[0] }) => (
    <TouchableOpacity style={[styles.card, { backgroundColor: item.cor }]} activeOpacity={0.8} onPress={() => navigation}>
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={{ flex: 1, opacity: fadeAnim }}>
        <FlatList data={PETS_DATA} keyExtractor={(item) => item.id} renderItem={renderPetItem} />
      </Animated.View>
    </View>
  );
}
